...
require 'ostruct'
require 'erb'
require File.expand_path(File.dirname(__FILE__) + '/simple_io')
...
class FileRobot
  ...
  def create_file_from_template(filename, template_file_name, the_binding)
    template_string = File.new(template_file_name).read
    content = ERB.new(template_string).result(the_binding)
